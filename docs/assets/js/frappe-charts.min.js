	...
	yAxis: {
		...
		makeElements: function makeElements(data) {
			...
			data.positions.forEach(function (position, i) {
				elements.push(yLine(position, data.labels[i], _this3.constants.width, {
					mode: _this3.constants.mode,
					pos: data.pos || _this3.constants.pos,
					shortenNumbers: _this3.constants.shortenNumbers
				}));
			});
	...
	xAxis: {
		...
		makeElements: function makeElements(data) {
			...
			return data.positions.map(function (position, i) {
				return xLine(position, data.calcLabels[i], _this5.constants.height, { mode: _this5.constants.mode, pos: _this5.constants.pos });
			});
		},
		animateElements: function animateElements(newData) {
			var newPos = newData.positions;
	...
	yRegions: {
		...
		makeElements: function makeElements(data) {
			...
			return data.map(function (r) {
				return yRegion(r.startPos, r.endPos, _this7.constants.width, r.label, { labelPos: r.options.labelPos });
			});
		},
		animateElements: function animateElements(newData) {
		...
		value: function calcYAxisParameters(dataValues) {
			...
			if (dataValues instanceof Array) {
				...
				var _loop = function _loop(key) {
					...
					yAxisConfigObject = _this2.config.yAxisConfig.find(function (item) {
						return key === item.id;
					}) || [];
					yAxisAlignment = yAxisConfigObject.position ? yAxisConfigObject.position : 'left';
			...
			var scaleAll = function scaleAll(values, id) {
				return values.map(function (val) {
					...
					if (yAxis instanceof Array) {
						yAxis = yAxis.length > 1 ? yAxis.find(function (axis) {
